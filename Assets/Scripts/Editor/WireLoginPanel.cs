...
public static class WireLoginPanel
{
    ...
    static void Wire()
    {
        ...
        SetupPanelLayout(panelGO);
        WireSerializedRefs(panelGO);
    }

    static void SetupPanelLayout(GameObject panelGO)
    {
        var panelRT = panelGO.GetComponent<RectTransform>();
        if (panelRT == null) panelRT = panelGO.AddComponent<RectTransform>();
        panelRT.anchorMin = panelRT.offsetMin = Vector2.zero;
        panelRT.anchorMax = panelRT.offsetMax = Vector2.one;

        var bg = panelGO.transform.Find("Background");
        if (bg != null)
        {
            var bgRT = EnsureRT(bg.gameObject);
            bgRT.anchorMin = bgRT.offsetMin = Vector2.zero;
            bgRT.anchorMax = bgRT.offsetMax = Vector2.one;
            var img = bg.GetComponent<Image>() ?? bg.gameObject.AddComponent<Image>();
            img.color = new Color(0.047f, 0.047f, 0.071f, 0.93f);
        }

        Vector2 mid = new Vector2(0.5f, 0.5f);
        void Place(string name, Vector2 pos, Vector2 size)
        {
            var t = panelGO.transform.Find(name); if (t == null) return;
            var rt = EnsureRT(t.gameObject); rt.anchorMin = rt.anchorMax = rt.pivot = mid;
            rt.anchoredPosition = pos; rt.sizeDelta = size;
        }

        Place("TitleText", new Vector2(0, 160), new Vector2(420, 50));
        SetupTitleText(panelGO);
    ...
    static void SetupTitleText(GameObject panelGO)
    {
        var titleT = panelGO.transform.Find("TitleText");
        if (titleT != null)
        {
            var tmp = titleT.GetComponent<TextMeshProUGUI>() ?? titleT.gameObject.AddComponent<TextMeshProUGUI>();
            tmp.text = "TERRAFORMATION"; tmp.fontSize = 28;
            tmp.alignment = TextAlignmentOptions.Center; tmp.color = new Color(0.392f, 0.706f, 1f, 1f);
        }
    }

    static void SetupStatusText(GameObject panelGO)
    {
        var statusT = panelGO.transform.Find("StatusText");
        if (statusT != null)
        {
            var tmp = statusT.GetComponent<TextMeshProUGUI>() ?? statusT.gameObject.AddComponent<TextMeshProUGUI>();
            tmp.fontSize = 13; tmp.alignment = TextAlignmentOptions.Center;
            tmp.color = new Color(0.549f, 0.549f, 0.549f, 1f); tmp.text = "";
        }
    }

    static void WireSerializedRefs(GameObject panelGO)
    {
        ...
        if (!Application.isPlaying) EditorSceneManager.MarkSceneDirty(panelGO.scene);
    ...
    static void SetupInputField(string childName, string placeholderText, TMP_InputField.ContentType contentType)
    {
        ...
        CreateTextArea(go, placeholderText, field);
        EditorUtility.SetDirty(go);
    }

    static void CreateTextArea(GameObject go, string placeholderText, TMP_InputField field)
    {
        ...
        phTMP.text = placeholderText; phTMP.fontSize = 14;
        phTMP.fontStyle = FontStyles.Italic; phTMP.color = new Color(0.5f, 0.5f, 0.5f, 0.75f);
        ...
        txtTMP.text = ""; txtTMP.fontSize = 14; txtTMP.color = Color.white;
